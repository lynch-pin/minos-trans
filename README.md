...
두 단계로 동작합니다. 둘 다 API 키도 서버도 필요 없습니다.

1. **기기 음성 (Web Speech API).** 브라우저·운영체제가 그리스어 음성을
   제공하면 그걸 우선 씁니다. macOS·Windows·Android·iOS 는 대개 그리스어
   음성이 있거나 설치할 수 있습니다. **설정 → 발음 음성** 에서 기기가 가진
   그리스어 음성을 보고 고를 수 있고, 그 자리에서 들어볼 수 있습니다.
   엉뚱한 언어로 읽힌다면 여기부터 확인하세요.
2. **미리 만든 오디오.** 기기에 그리스어 음성이 없으면(리눅스 데스크톱 등에서
   흔합니다) `data/audio/` 에 미리 만들어 둔 mp3 로 대신합니다. 제작 시점에
   `espeak-ng` 로 모든 문장·낱말의 발음을 만들어 레포에 넣어 둔 것이라
   방문자의 기기 상태와 무관하게 항상 재생됩니다 — 그래서 듣기 버튼이
   기기에 따라 사라지는 일이 없습니다. 다만 기계 합성 음성이라 억양이
   자연스럽지 않을 수 있습니다.

새 문장을 추가한 뒤에는 `python3 tools/gen-audio.py` 를 돌려야 그 문장의
mp3 가 생기고 듣기 버튼이 나타납니다 (`CLAUDE.md` 참고).
